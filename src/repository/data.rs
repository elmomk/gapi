...
fn col<T: rusqlite::types::FromSql>(row: &rusqlite::Row, name: &str) -> rusqlite::Result<T> {
    row.get(row.as_ref().column_index(name)?)
}

fn row_to_daily_data(row: &rusqlite::Row) -> rusqlite::Result<GarminDailyData> {
    let user_id_str: String = col(row, "user_id")?;
    let date_str: String = col(row, "date")?;
    let synced_at: f64 = col(row, "synced_at")?;
    ...
    Ok(GarminDailyData {
        ...
        steps: col(row, "steps")?,
        distance_meters: col(row, "distance_meters")?,
        active_calories: col(row, "active_calories")?,
        total_calories: col(row, "total_calories")?,
        floors_climbed: col(row, "floors_climbed")?,
        intensity_minutes: col(row, "intensity_minutes")?,
        resting_heart_rate: col(row, "resting_heart_rate")?,
        max_heart_rate: col(row, "max_heart_rate")?,
        min_heart_rate: col(row, "min_heart_rate")?,
        avg_heart_rate: col(row, "avg_heart_rate")?,
        hrv_weekly_avg: col(row, "hrv_weekly_avg")?,
        hrv_last_night: col(row, "hrv_last_night")?,
        hrv_status: col(row, "hrv_status")?,
        sleep_score: col(row, "sleep_score")?,
        sleep_duration_secs: col(row, "sleep_duration_secs")?,
        deep_sleep_secs: col(row, "deep_sleep_secs")?,
        light_sleep_secs: col(row, "light_sleep_secs")?,
        rem_sleep_secs: col(row, "rem_sleep_secs")?,
        awake_secs: col(row, "awake_secs")?,
        avg_stress: col(row, "avg_stress")?,
        max_stress: col(row, "max_stress")?,
        body_battery_high: col(row, "body_battery_high")?,
        body_battery_low: col(row, "body_battery_low")?,
        body_battery_drain: col(row, "body_battery_drain")?,
        body_battery_charge: col(row, "body_battery_charge")?,
        weight_grams: col(row, "weight_grams")?,
        bmi: col(row, "bmi")?,
        body_fat_pct: col(row, "body_fat_pct")?,
        muscle_mass_grams: col(row, "muscle_mass_grams")?,
        avg_spo2: col(row, "avg_spo2")?,
        lowest_spo2: col(row, "lowest_spo2")?,
        avg_respiration: col(row, "avg_respiration")?,
        training_readiness: col(row, "training_readiness")?,
        training_load: col(row, "training_load")?,
        vo2_max: col(row, "vo2_max")?,
        activities_count: col(row, "activities_count")?,
        activities_json: col(row, "activities_json")?,
        sleep_restless_moments: col(row, "sleep_restless_moments")?,
        sleep_avg_overnight_hr: col(row, "sleep_avg_overnight_hr")?,
        skin_temp_overnight: col(row, "skin_temp_overnight")?,
        sleep_score_feedback: col(row, "sleep_score_feedback")?,
        training_readiness_feedback: col(row, "training_readiness_feedback")?,
